...
class Core
{
    ...
    {
        // Nếu đã cache channel thì trả luôn
        if ($this->currentChannel) {
            return $this->currentChannel;
        }

        // Nếu không có hostname truyền vào, lấy từ request hoặc .env
        if (! $hostname) {
            if (app()->runningInConsole()) {
                $hostname = config('app.default_channel_hostname', 'localhost');
            } else {
                $hostname = request()->getHttpHost();
            }
        }

        // Tìm channel khớp hostname
        ...
        // Nếu không tìm thấy thì fallback sang channel mặc định
        if (! $this->currentChannel) {
            $this->currentChannel = $this->channelRepository->first();
        }

        // Nếu vẫn không có channel nào → báo lỗi rõ ràng
        if (! $this->currentChannel) {
            throw new \Exception("No channel found for hostname: {$hostname}");
        }
