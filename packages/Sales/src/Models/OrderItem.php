...
class OrderItem extends Model implements OrderItemContract
{
    ...
    public function getTypeInstance(): AbstractType
    {
        ...
        // Skip product setting for course orders to avoid Product class loading
        // if ($this->product) {
    ...
    {
        // Course orders are not stockable (no physical inventory)
        return false;
    }
    ...
    {
        // Course orders cannot be shipped
        return false;
    }
    ...
    {
        // Create a safe morphTo that never actually loads the Product class
        // by using a dummy model class that exists
        return $this->morphTo('product', 'type', 'id')
            ->where('id', -999); // Non-existent ID to ensure no results
    }
    
    /**
     * Get product attribute for safe access in views
     */
